...
import dev.forge.core.model.TextPart
 ...
 * Forge/opencode payload shapes:
 *
 *  - Every SSE frame uses the `event: message` name; the typed event lives in
 *    the JSON `data` body (Forge internal/server/sse.go writeSSE).
...
class SseWireParsingTest {
    ...
    fun `message_part_updated reads nested part object`() {
        // Forge: properties = {sessionID, part:{...}, time}
        val raw = sse(
    ...
    fun `message_part_delta reads partID not id`() {
        // Forge: properties = {sessionID, messageID, partID, field, delta}
        val raw = sse(
            "message.part.delta",
    ...
    private fun sse(type: String, propsJson: String): SseEvent =
        SseEvent(
            id = "evt",
            type = type,
            properties = ForgeJsonParse(propsJson),
        )
}

/** Minimal helper to build a JsonObject from a raw JSON string in tests. */
private fun ForgeJsonParse(s: String) =

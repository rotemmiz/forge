...
- **Wordmark** (`forge` on splash): blocky pixel font *Silkscreen* 76px, grey→white vertical gradient (`#6b7178 → #d6dade → #ffffff`). In a terminal, render as ASCII/figlet block-letters with a grey gradient if supported, else bold.
...
- Spacer, then footer: cwd path + `fixture:main` (`--fg-dim`) and `• Forge 0.4.2` (version, `--fg-faint`, name bold).
...
- **Model switcher** (`ctrl+x m` or `/models`): grouped by provider (`Forge Cloud`, `Anthropic`, `OpenAI`, `Google`); current model marked with leading `●`; free models tagged `Free` (right). Footer: `Connect provider ctrl+a   Favorite ctrl+f`.
...
- `Forge TUI.html` — entry; loads React/Babel + the scripts below. Supports `?view=<name>` to deep-link a screen state (used by the gallery): `splash, chat, tools, output, summary, palette, models, agents, themes, sessions, timeline, status`.
...
- `Forge Screens.html` + `design-canvas.jsx` — a gallery that embeds every screen state side-by-side for review.

To browse all screens at once, open `Forge Screens.html`. To explore the live app, open `Forge TUI.html` (type a prompt, try `ctrl+p`, `tab`, `/`, `@`, `ctrl+x` then a key).
